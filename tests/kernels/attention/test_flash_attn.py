...
def test_varlen_with_paged_kv_pytorch_reference(
    ...
    """pytorch_paged_attention must match this suite's golden reference
    (ref_paged_attn) on the plain causal path across the kernel's own
    parameter matrix."""
...
@pytest.mark.parametrize("num_blocks", NUM_BLOCKS)
@torch.inference_mode()
def test_varlen_with_paged_kv_pytorch_int8_dynamic(
    seq_lens: list[tuple[int, int]],
    num_heads: tuple[int, int],
    head_size: int,
    dtype: torch.dtype,
    ...
    """Fully dynamic INT8 (Q and gathered KV quantized at read time) must
    stay within a bounded distance of the unquantized reference."""
    from vllm.v1.attention.backends.pytorch_paged_ref import (
    ...
    query = torch.randn(sum(query_lens), num_heads[0], head_size, dtype=dtype)
    key_cache = torch.randn(
        num_blocks, block_size, num_heads[1], head_size, dtype=dtype
    )
    value_cache = torch.randn_like(key_cache)
    cu_query_lens = torch.tensor([0] + query_lens, dtype=torch.int32).cumsum(
        dim=0, dtype=torch.int32
